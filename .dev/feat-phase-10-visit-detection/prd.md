...
- [Must] **FR-VD-10**: 토스트에는 장소명과 주소를 표시한다. 장소명 앞에 📍 아이콘. 형식: `📍 {장소명} 근처에 계신가요?` + 주소 부줄. 주소가 null이거나 빈 문자열인 경우 주소 라인을 표시하지 않는다 (장소명만 표시).
...
- [Must] **FR-VD-15**: 1차 PATCH 성공 시 마커 confetti 애니메이션을 실행한다. 아래 두 시간은 별개 기준으로 동시에 진행된다.
  - 하트 ♡ 3개 fan-out: **개별 하트 fade-out 200ms** (마커 위치에서 위쪽 랜덤 경로로 떠오르며 사라짐)
  - 마커 scale `1.0 → 1.3 → 1.0` 이중 bounce: **마커 전체 연출 600ms**
  - 마커 아이콘은 confetti 시작과 동시에 WISH/REEL → MEMORY 아이콘으로 교체
  - **전체 연출 종료 기준: 600ms** (마커 bounce 완료 시점). 하트는 그 안에서 200ms 만에 먼저 사라진다.
...
| AC-VD-6 | 토스트에 장소명(📍 {장소명} 근처에 계신가요?)과 주소가 표시된다. 주소 null인 경우는 라인 생략. | FR-VD-10 |
| AC-VD-7 | "다음에 올게요" 클릭 시 토스트가 닫히고 세션 Set에 pinId가 추가된다 | FR-VD-12 |
| AC-VD-8 | "네, 다녀왔어요 →" 클릭 시 즉시 태그 PATCH가 발사된다 | FR-VD-14 |
| AC-VD-9 | 태그 PATCH 성공 시 마커에서 하트 confetti(3개, fan-out, ~200ms) + scale bounce가 동시에 실행되며 마커 아이콘이 MEMORY로 교체된다. 하트 fade-out 200ms와 마커 bounce 600ms는 별개 시간 기준임. | FR-VD-15 |
