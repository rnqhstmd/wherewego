 ...
 * @param lastReadMessageId 이번 요청으로 읽음 포인터가 전진하기 "직전"의 내 포인터(행 없음/방 없음 {@code null}).
 *                          클라이언트가 "안 읽은 위치부터 진입" 앵커를 서버 진실로 계산한다
 *                          (목록 unreadCount 는 목록 로드 시점 스냅샷이라 스테일 앵커를 만든다).
 */
public record GroupMessagesPage(
    ...
    public static GroupMessagesPage empty() {
        return new GroupMessagesPage(List.of(), false, null, null);
    }
}
