...
2. **`.env` 생성**
   본 저장소는 `.env`를 추적하지 않습니다(보안). 새 개발자는 아래 키 목록을 참조하여 `backend/.env`를 직접 작성합니다:
   ```
   SPRING_PROFILES_ACTIVE, POSTGRES_HOST/PORT/DB/USER/PASSWORD,
   KAKAO_CLIENT_ID/SECRET/REDIRECT_URI, KAKAO_LOCAL_API_KEY,
   GOOGLE_PLACES_API_KEY, MAPBOX_TOKEN,
   JWT_SECRET, JWT_ACCESS_TTL_SECONDS=3600, JWT_REFRESH_TTL_SECONDS=1209600,
   WEB_SECURITY_COOKIE_SECURE/DOMAIN/SAME_SITE, CORS_ALLOWED_ORIGINS,
   KAKAO_SKILL_SECRET, KAKAO_BOT_ID, SLACK_WEBHOOK_URI/USERNAME/CHANNEL
   ```
