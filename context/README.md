...
| **Phase 8** | **인앱 알림함**: 그룹원 핀 등록 시 상대방 알림 생성 (릴스 1건 = 알림 1건 묶음). 유형: `CHATBOT_PINS`(릴스/장소 선택, 복수 핀) / `MANUAL_PIN`(웹 직접 등록). **실시간(SSE)**: `SseEmitter` Registry + 30초 heartbeat + 5분 timeout, 사용자당 최대 10개 emitter DoS 가드. **프론트 UX**: 모바일 우상단 [벨][프로필] 가로 배치 + 데스크탑 사이드바 벨 슬롯. 빨간 점(미읽음) / 회색 점(연결 끊김 + tooltip), 5초 자동 닫힘 + 외부 탭 말풍선, 패널 → 핀 클릭 시 지도 `flyTo` + `PinPopup`. 백엔드: `notifications` + `notification_pins` 테이블(V007), `receiver_id` 단일 컬럼 fan-out, `NotificationService` @Transactional + `@TransactionalEventListener(AFTER_COMMIT)` SSE push, BFF 전용 SSE 라우트 + preflight 401 즉시 failed. 호출자 try-catch로 BR-3 트랜잭션 격리. AC-1~AC-22 충족 | ✅ 완료 | [#40](https://github.com/rnqhstmd/wherewego/pull/40) |
