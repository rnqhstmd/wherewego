...
| **chat** | **2026-06-10 신설**: 그룹 채팅방 + 릴스 공유·온디맨드 장소 등록 — 봇 티키타카(모아보기) 대체. GC-1(백엔드 ✅ PR #118)/GC-2(iOS)/GC-3(정리+썸네일) 3 Phase | [상세](chat/README.md) |
...
| **tag** | **Phase 7 갱신**: REEL·발견(하늘색 `#7BB3E8` 동그라미) / WISH·설렘(노랑 `#F4C842` 별모양) / MEMORY·추억(핑크 `#FFB3C6` 하트) 3종 카테고리 | [상세](tag/README.md) |
...
| **notification** | **Phase 8 신설**: 그룹원 핀 등록 시 상대방 인앱 알림 (mount/visibility fetch + 벨/패널 UI, MANUAL_PIN/CHATBOT_PINS). 옵션 B 다운그레이드(2026-05-21)로 SSE 인프라 제거, 사용자 100명+ 시점 재평가 | [상세](notification/README.md) |
...
| **Phase 7** | **태그 3종 리뉴얼**: PLACE→REEL(발견/하늘색 `#7BB3E8`/동그라미)·WISH(설렘/노랑 `#F4C842`/별모양) 분리, MEMORY(추억/핑크 `#FFB3C6`/하트) 유지. **V006 단일 합본 Flyway 마이그레이션**(ALTER+UPDATE+ALTER 단일 트랜잭션), 챗봇 기본값 REEL, 웹 등록 UI WISH/MEMORY 2종 선택(REEL 챗봇 전용·핀 편집은 3종 모두 허용), 지도 마커 3종 신설(공통 SVG 모듈 `lib/pin/markers.tsx`), 룰렛 후보 풀 REEL+WISH + MapClient 토글 부분버그(PR #17 잔존) 정합화 | ✅ 완료 | [#38](https://github.com/rnqhstmd/wherewego/pull/38) |
| **Phase 8** | **인앱 알림함**: 그룹원 핀 등록 시 상대방 알림 생성 (릴스 1건 = 알림 1건 묶음). 유형: `CHATBOT_PINS`(릴스/장소 선택, 복수 핀) / `MANUAL_PIN`(웹 직접 등록). **fetch 트리거(옵션 B 다운그레이드, 2026-05-21)**: SSE 인프라 제거, `useNotifications` 훅이 mount + `visibilitychange` + `focus` 이벤트에서 `GET /notifications` 호출. 앱 다시 켤 때나 새로고침 시 빨간 점/패널 갱신. **프론트 UX**: 모바일 우상단 [벨][프로필] 가로 배치 + 데스크탑 사이드바 벨 슬롯. 빨간 점(미읽음), visibilitychange 시 직전 max id 초과한 신규 알림 감지 시 5초 말풍선(외부 탭 닫힘), 패널 → 핀 클릭 시 지도 `flyTo` + `PinPopup`. 백엔드: `notifications` + `notification_pins` 테이블(V007), `receiver_id` 단일 컬럼 fan-out, `NotificationService` @Transactional + 호출자 try-catch로 BR-3 트랜잭션 격리. SSE 의존 항목(FR-5~9/15/17, BR-6)은 ⬜ 보류, 사용자 100명+ 시점 재평가 | ✅ 완료 | [#40](https://github.com/rnqhstmd/wherewego/pull/40) |
| **Phase 9** | **핀 공유 카드**: 말풍선 팝업 ⋮ 좌측 sibling 공유 아이콘 → 바텀시트 1080×1350(4:5) Canvas 카드 미리보기 + "복사하기"(`navigator.clipboard.write` ClipboardItem) + "이미지 저장"(`<a download>`) 두 버튼. 배경: Mapbox Static API + off-canvas 2단계 `blur(8px)`. 레이아웃: 메모(히어로 44px) → 장소명(Bold 36px) → 날짜·주소 → `written by {닉네임}` → 좌하단 "우리가갈지도". Web Share API 미사용. Canvas 메모리 즉시 해제 + Pretendard self-host `@font-face` alias 등록 + BR-6 단색 폴백. lib/share 3 모듈 + 단위 테스트 40건. 백엔드/DB/S3 변경 0 | ✅ 완료 | [#41](https://github.com/rnqhstmd/wherewego/pull/41) |
| **Phase 10** | **장소 방문 감지**: WISH·REEL 핀 100m 이내 진입 시 "이 장소에 오셨나요?" 바텀 토스트 → 확인 시 MEMORY 전환 애니메이션 + 메모 바텀시트. 세션 중 동일 핀 중복 표시 없음. 프론트 Haversine 계산, 백엔드 신규 API 없음. [상세](pin/phase-10-visit-detection.md) | ✅ 완료 | [#57](https://github.com/rnqhstmd/wherewego/pull/57) |
...
| **GC-1** | **그룹 채팅 백엔드 기반**([chat](chat/README.md)): `chat_room` GROUP 일반화 + `REEL_LINK` kind + 멤버별 읽음(`chat_room_reads`, V021) + **registered 파생**(pins EXISTS 배치) + 온디맨드 추출 API(deadline 완화) + 방 목록 API + 푸시 일반화. 봇 흐름 무변경(병행 운영). FR 상세: [chat/status.md](chat/status.md) | ✅ 코드 완료 | [#118](https://github.com/rnqhstmd/wherewego/pull/118) |
