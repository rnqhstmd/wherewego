...
- **Phase 8 예정**: 인앱 알림 트리거 (웹 직접 등록 경로) — `PinService.addPin` 완료 후 `NotificationService.createManualNotification(groupId, registeredBy, pinId)` 호출. 단건 핀 = 알림 1건, 유형 `MANUAL_PIN`
- **Phase 9 예정**: 핀 공유 카드 — 지도 말풍선 팝업 ⋮ 메뉴에 "공유하기" 항목 추가. 클릭 시 바텀시트 오픈 → HTML Canvas로 카드 렌더링(Mapbox Static API 지도 이미지 배경 + CSS `filter: blur` 흐림 처리 + 메모 + 장소명 + 핀 등록일 + `written by {닉네임}` + 좌측 하단 "우리가갈지도" 워터마크) → `canvas.toBlob` PNG 추출 → Web Share API(`navigator.share`) 우선, 미지원 시 `<a download>` 폴백. 완전 클라이언트 처리, S3·서버 변경 없음
