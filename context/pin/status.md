...
| FR-PIN-7 | 핀 장소 좌표 수정 (백엔드 API) | ✅ | [PR-LINK] — `PATCH .../pins/{pinId}` 에 `latitude/longitude` BigDecimal 직접 필드 + `PinUpdateCommand.coordinateProvided` 단일 플래그, 범위 검증, 한쪽만 전달 시 `PIN_COORDINATE_INVALID`(400) |
| FR-PIN-8 | 핀 장소 좌표 수정 (지도 picker UX) | ✅ | [PR-LINK] — `PinPopup` ⋮ "좌표 수정" 진입점 → `PinCoordinateEditPicker` 시트 → `useOptimistic patch` 즉시 갱신 + 실패 시 자동 롤백 + 인라인 에러 |
