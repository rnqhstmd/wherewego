...
| FR-OBS-8 | Google Places Micrometer 메트릭 (outcome별 Counter/Timer, Gemini 패턴 복제) | ✅ | [#29](https://github.com/rnqhstmd/wherewego/pull/29) — Phase 2.11 PR-B. `GooglePlacesMetrics` 신규. `google_places.calls.total{outcome}` Counter + `google_places.call.duration{outcome}` Timer. outcome: success/empty/rate_limited/timeout/error/cached. AC-1 충족 |
| FR-OBS-9 | Google Places 응답 Caffeine 캐시 (24h, 키워드 SHA-256 키) | ✅ | [#29](https://github.com/rnqhstmd/wherewego/pull/29) — Phase 2.11 PR-B. `GooglePlacesResponseCacheService` 신규(SHA-256(keyword), put/get `List.copyOf`). 24h TTL, maximumSize 1,000. success/empty만 캐싱(rate_limited/timeout/error 제외). 캐시 hit 시 `cache=hit`, miss+put 성공 시 `cache=miss`. AC-2 ~ AC-5, AC-19 충족 |
| FR-OBS-10 | 1시간 합계 임계값 스케줄러 (`@Scheduled` 정시. Gemini server_error 10%/1h + 5분 쿨다운) | ✅ | [#29](https://github.com/rnqhstmd/wherewego/pull/29) — Phase 2.11 PR-B. `ThresholdMonitorScheduler`(`infrastructure.monitoring` 신규 패키지). `@Scheduled(fixedRate=1h, initialDelay=1h)`. MeterRegistry counters 스냅샷 + delta 차분. 분모는 `disabled` 제외, 분모 < 1 스킵. **Google Places 80%/95% 부분은 Phase 3 보류** (트래픽 30건/일로 8K 도달 불가, FR-OBS-14 rate limit과 함께 재검토). AC-6 ~ AC-10 충족 |
| FR-OBS-11 | Instagram scraper 3-stage 최종 실패율 추적 + 차단 감지 알림 (고정 1시간 윈도우, 50% 임계값 + 5분 쿨다운 → notifyFailure, 호출 0건 시 알림 생략) | ✅ | [#29](https://github.com/rnqhstmd/wherewego/pull/29) — Phase 2.11 PR-B. `InstagramBlockedRateTracker` 신규(synchronized 단일 락으로 attempts/blocked/lastBlockedUrl 원자 스왑). `InstagramScraperClient.fetchHtml()` finally의 `recordAttempt()` + BLOCKED 시 `recordBlocked(url)`. ThresholdMonitorScheduler.flushWindow() 호출로 윈도우 종료 처리. AC-11 ~ AC-14, AC-18 충족 |
