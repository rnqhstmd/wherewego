...
| 📍 이모지 우선 추출 (Phase 2, 폐기) | 캡션 내 핀 이모지 뒤 텍스트를 1순위 장소명 후보로 사용했던 regex. **Phase 2.5에서 Gemini로 대체 완료 ([#15](https://github.com/rnqhstmd/wherewego/pull/15))** |
| 위치/장소 키워드 (Phase 2, 폐기) | 2순위 추출 규칙. `장소:`, `at @`, `in @` 뒤 단어. **Phase 2.5에서 Gemini로 대체 완료** |
| 해시태그 폴백 (Phase 2, 폐기) | 3순위 추출 규칙. `#장소명` 형태에서 추출. **Phase 2.5에서 Gemini로 대체 완료** |
| Gemini 2.0 Flash (Phase 2.5) | Google AI Studio LLM. 캡션 텍스트 → 장소명 1개 추출. 무료 1,500건/일. `timeout-ms=3000`. 인증 `x-goog-api-key` 헤더 |
| CaptionCleaner (Phase 2.5) | og:description 앞부분(likes/comments) 제거 정제기. regex `/:\s*"(.+)"\.?\s*$/` DOTALL. 매칭 실패 시 원문 trim 반환 |
| GeminiPlaceClient (Phase 2.5) | Gemini 2.0 Flash REST API 호출 클라이언트. `Optional<String>` 반환. 타임아웃/429/응답 `"null"` 모두 empty로 수렴 |
| CAPTION_MAX_LENGTH (Phase 2.5) | Gemini 호출 전 캡션 절단 상한(500자). 비용 폭발 + 프롬프트 인젝션 면적 가드 |
| Gemini 호출 직전 ctx.expired() 가드 (Phase 2.5) | scraper 4s + gemini 3s가 챗봇 5초 SLA(deadlineMs=4500)를 초과할 수 있으므로 호출 직전 추가 컷오프 |
