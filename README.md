...
하단 검색 탭에서 장소명을 검색하고 발견·설렘·추억 태그를 선택해 핀을 등록합니다. 카카오톡 챗봇에 인스타 릴스 링크를 보내면 AI가 장소를 자동 추출하고, 링크 전송 후 1분 내 메모를 입력하면 핀에 메모가 함께 저장됩니다.

### 지도 탐색

| 지도 줌인·줌아웃 & 3D 지구본 |
|:---:|
| <img src="docs/demo/map-globe-cluster.gif" alt="지도 줌인·줌아웃" width="280" /> |
...
핀 팝업의 더보기(⋮)에서 장소명·메모·태그 수정과 삭제가 가능합니다. 공유 아이콘을 탭하면 Mapbox 배경 위에 핀 정보가 담긴 4:5 카드 이미지가 생성되어 클립보드에 복사되거나 저장됩니다.
...
하단 "어디갈까?" 버튼을 탭하면 현재 위치 10km 이내의 발견·설렘 핀 중 하나를 랜덤 추천합니다. 현재 위치 100m 이내에 저장된 핀이 감지되면 방문 토스트가 표시되며, "네 다녀왔어요"를 선택하면 해당 핀이 추억 태그로 전환됩니다.
