...
struct PinBubbleView: View {
    ...
    var body: some View {
        ZStack {
            // 전체화면 투명 배경탭(BR-3): 바깥 탭 시 닫기. 단, 사진 업로드/삭제 중이면 닫기 무시(silent 실패 방지).
            // ZStack 전체에 깔려 `.position` 영향을 받지 않는다 — 말풍선 본체와 적용 단위 분리(AC-2).
            Color.clear
                .contentShape(Rectangle())
                .ignoresSafeArea()
                .onTapGesture {
                    guard !detailVM.isPhotoBusy else { return }
                    closeBubble()
                }
    ...
    private var bubble: some View {
        VStack(spacing: 0) {
            ...
            // 본체: 흰 카드 + 라운드 + 테두리 + 그림자(웹 SpeechBubblePopup 컨테이너 동치).
            ...
            // 꼬리(아래 방향) — 본체 하단 중앙. 웹 svg 22x12 `M 0 0 L 11 11 L 22 0 Z` 동치.
            BubbleTail()
                .fill(WGColor.panel)
